...
# injected to check on self-reported GPU use
import tensorflow as tf
sess = tf.Session(config=tf.ConfigProto(log_device_placement=True))


# set up experiment logging set COMET_API_KEY in your environment variables or pass it as the first value in the
# Experiment object
...
# partition into train and test DataFrames manually
X_meta = pd.read_csv(metadata_filepath)
X = X_meta['CroppedImageURL']
y = X_meta['LabelName']
split_ratio = 0.8
n_samples = len(X)
...
X_train_df = pd.DataFrame().assign(ImagePath=X_train, ImageClass=y_train)
X_test_df = pd.DataFrame().assign(ImagePath=X_test, ImageClass=y_test)


# manully decrease the size of the training data
# X_train_df_s = X_train_df.sample(5000) X_test_df_s = X_train_df.sample(2000)
...
# compile the model
model.compile(
    # 1/50 of the standard SGD learning rate w/ lots of momentum
